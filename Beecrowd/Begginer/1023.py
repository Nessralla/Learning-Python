...
while True:
  ...
  # Senão criamos uma lista para receber os valores de habitantes e consumo da cidade
    ...
    for i in range(testCase):
        ...
        RelCid[cidade][i].append(int(temp[0]))
        RelCid[cidade][i].append(int(temp[1]))
...
for cid in RelCid:
  ...
  for i in range(len(RelCid[cid])):
    listProv[0].append(RelCid[cid][i][0])
    listProv[1].append(RelCid[cid][i][2])
    listProv[2].append(RelCid[cid][i][1])
  ...
  while len(listProv[1]) > 0:
    ...
    if a > 1:
        for i in range(a):
            ...
            del listProv[0][ind]
            del listProv[1][ind]
        print(f"{sum(listDuplic)}-{listProv[1][ind]}",end=' ')
